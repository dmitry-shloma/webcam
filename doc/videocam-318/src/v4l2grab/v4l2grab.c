...
/**
	process image read
*/
static void imageProcess(const void* p, struct timeval timestamp, const char *jpegFilename)
{
	//timestamp.tv_sec
	//timestamp.tv_usec
...
/**
	read single frame
*/
static int frameRead(const char *jpegFilename)
{
...
*/
void mainLoop(const char *jpegFilename)
{	
	int count;
...
#ifdef IO_MMAP
static void mmapInit(const char *deviceName)
{
...
#ifdef IO_USERPTR
static void userptrInit(unsigned int buffer_size, const char *deviceName)
{
	struct v4l2_requestbuffers req;
...
/**
	initialize device
*/
void deviceInit(const char *deviceName)
{
...
/**
	open device
*/
void deviceOpen(const char *deviceName)
{
	struct stat st;
